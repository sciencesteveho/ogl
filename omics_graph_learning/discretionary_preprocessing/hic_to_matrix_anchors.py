...
def _split_hicdcplus_to_chrs(
    input_dir: pathlib.PosixPath, input: str, qvalue_cutoff: float
) -> None:
    """Lorem"""
        ...
        for row in reader:
            if float(row["qvalue"]) <= qvalue_cutoff:
                ...
                if chr_file_name not in opened_files:
                    ...
                    opened_files[chr_file_name] = csv.writer(
...
def _filtered_hic_to_sparse_matrix(input: str, chr: str) -> None:
    ...
    # read in the data
    hic_df = pd.read_csv(
        input, header=None, delimiter="\t", names=["start_i", "start_j", "count"]
    )
...
def process_matrices(
    input: str, qval: float, chr_list: List[str], chr_lens: List[int]
) -> None:
    """Lorem"""
    # get seq files
