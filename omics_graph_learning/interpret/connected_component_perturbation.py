...
class ConnectedComponentPerturbation:
    ...
    def _perform_joint_re_perturbations(
        ...
        result = self._perform_random_group_removal(
            sub_data=sub_data,
            gene_node=gene_node,
...
# ) -> Dict[str, Any]:
#     """Compute the impact of removing nodes from connected components on model
#     output.

#     For each gene in `genes_to_analyze`:
#       1) Build subgraph around gene
#       2) Compute baseline
#       3) Single-node perturbations up to `max_nodes_to_perturb` - store
#          fold_change + hop_distance
#       4) If >=3 enhancers - randomly remove 2 => store fold_change, randomly
#          remove 3 => store
#       5) If >=3 promoters - same as enhancers
...
#     for gene_node in tqdm(genes_to_analyze, desc="Connected Component Perturbation"):
#         gene_id = idxs_inv.get(gene_node, str(gene_node))

#         # create subgraph loader
...
#         # exp 1: perform single node perturbations of the gene-centered subgraph
#         for node_remove in selected_nodes:
#             perturb_prediction = remove_nodes_and_predict(
#                 runner=runner,
#                 sub_data=sub_data,
#                 nodes_to_remove=[node_remove],
#                 gene_node=gene_node,
#                 device=device,
#                 mask_attr=mask_attr,
#             )
...
#             # store fold change and hop distance
#             node_name = idxs_inv.get(node_remove, str(node_remove))
#             hop_distance = lengths.get(node_remove, -1)
#             perturbations["single"][node_name] = {
#                 "fold_change": fold_change,
#                 "hop_distance": hop_distance,
#             }

#         # exp 2: enhancer removal
#         enhancers = get_elements_in_subgraph(
#             sub_data=sub_data,
#             idxs_inv=idxs_inv,
#             regulatory_element="_enhancer",
#         )
#         if len(enhancers) >= 3:
#             two_perturb = random.sample(enhancers, 2)
#             two_prediction = remove_nodes_and_predict(
#                 runner=runner,
#                 sub_data=sub_data,
#                 nodes_to_remove=two_perturb,
#                 gene_node=gene_node,
#                 device=device,
#                 mask_attr=mask_attr,
#             )
#             if two_prediction is not None:
#                 two_fc = calculate_log2_fold_change(
#                     baseline_prediction=baseline_prediction,
#                     perturbation_prediction=two_prediction,
#                 )
#                 two_enhancer_names = tuple(
#                     sorted([idxs_inv.get(eid, str(eid)) for eid in two_perturb])
#                 )
#                 perturbations["double_enhancers"][two_enhancer_names] = two_fc

#             three_perturb = random.sample(enhancers, 3)
#             three_prediction = remove_nodes_and_predict(
#                 runner=runner,
#                 sub_data=sub_data,
#                 nodes_to_remove=three_perturb,
#                 gene_node=gene_node,
#                 device=device,
#                 mask_attr=mask_attr,
#             )
#             if three_prediction is not None:
#                 three_fc = calculate_log2_fold_change(
#                     baseline_prediction=baseline_prediction,
#                     perturbation_prediction=three_prediction,
#                 )
#                 three_enhancer_names = tuple(
#                     sorted([idxs_inv.get(eid, str(eid)) for eid in three_perturb])
#                 )
#                 perturbations["triple_enhancers"][three_enhancer_names] = three_fc

#         # exp 3: promoter removal
#         promoters = get_elements_in_subgraph(
#             sub_data=sub_data,
#             idxs_inv=idxs_inv,
#             regulatory_element="_promoter",
#         )
#         if len(promoters) >= 3:
#             two_perturb = random.sample(promoters, 2)
#             two_prediction = remove_nodes_and_predict(
#                 runner=runner,
#                 sub_data=sub_data,
#                 nodes_to_remove=two_perturb,
...
#                     perturbation_prediction=two_prediction,
#                 )
#                 two_promoter_names = tuple(
#                     sorted([idxs_inv.get(eid, str(eid)) for eid in two_perturb])
#                 )
#                 perturbations["double_promoters"][two_promoter_names] = two_fc

#             three_perturb = random.sample(promoters, 3)
#             three_prediction = remove_nodes_and_predict(
#                 runner=runner,
#                 sub_data=sub_data,
#                 nodes_to_remove=three_perturb,
...
#                     perturbation_prediction=three_prediction,
#                 )
#                 three_promoter_names = tuple(
#                     sorted([idxs_inv.get(eid, str(eid)) for eid in three_perturb])
#                 )
#                 perturbations["triple_promoters"][three_promoter_names] = three_fc

#         # exp 4: dyadic removal
#         dyadics = get_elements_in_subgraph(
#             sub_data=sub_data,
#             idxs_inv=idxs_inv,
#             regulatory_element="_dyadic",
#         )
#         if len(dyadics) >= 3:
#             two_perturb = random.sample(dyadics, 2)
#             two_prediction = remove_nodes_and_predict(
#                 runner=runner,
#                 sub_data=sub_data,
#                 nodes_to_remove=two_perturb,
...
#                     perturbation_prediction=two_prediction,
#                 )
#                 two_dyadic_names = tuple(
#                     sorted([idxs_inv.get(eid, str(eid)) for eid in two_perturb])
#                 )
#                 perturbations["double_dyadics"][two_dyadic_names] = two_fc

#             three_perturb = random.sample(dyadics, 3)
#             three_prediction = remove_nodes_and_predict(
#                 runner=runner,
#                 sub_data=sub_data,
#                 nodes_to_remove=three_perturb,
...
#                     perturbation_prediction=three_prediction,
#                 )
#                 three_dyadic_names = tuple(
#                     sorted([idxs_inv.get(eid, str(eid)) for eid in three_perturb])
#                 )
#                 perturbations["triple_dyadics"][three_dyadic_names] = three_fc
