...
class GraphConstructor:
    ...
    def _tf_markers(self, interaction_file: str) -> List[Tuple[str, str]]:
        ...
        return [
            (
                self.genesymbol_to_gencode[tup[0]],
    ...
    def _marbach_regulatory_circuits(
        ...
        with open(interaction_file, newline="") as file:
            return [
                ...
                if line[0] in self.genesymbol_to_gencode.keys()
                and line[1] in self.genesymbol_to_gencode.keys()
            ]

    def _enhancer_index(self, e_index: str, e_index_unlifted: str) -> Dict[str, str]:
        """Returns a dict to map enhancers from hg19 to hg38"""

        def text_to_dict(txt, idx1, idx2):
            with open(txt) as file:
                file_reader = csv.reader(file, delimiter="\t")
                return {line[idx1]: line[idx2] for line in file_reader}
        ...
        return {k: v for k, v in e_dict_unfiltered.items() if "alt" not in v}
    ...
    @time_decorator(print_args=True)
    def _polyadenylation_targets(self, interaction_file: str) -> List[str]:
    ...
    def _prepare_graph_tensors(
        ...
        def _reindex_nodes(edges):
            ...
            edges_reindexed = list(
                map(
                    lambda edge: [node_idxs[edge[0]], node_idxs[edge[1]], edge[2]],
                    edges,
                )
            )
        ...
        edges_to_add = [
            ...
            for line in filter(
                lambda interaction: interaction[0] in nodes_to_add
                or interaction[1] in nodes_to_add,
                interaction_edges,
            )
        ]
        ...
        with open(gene_edges, newline="") as file:
            local_edges = [
                [line[3], line[7], "local"] for line in csv.reader(file, delimiter="\t")
            ]
            ...
            pickle.dump(
                {
                    "edge_index": np.array(
                        [
                            [edge[0] for edge in edges_reindexed],
                            [edge[1] for edge in edges_reindexed],
                        ]
                    ),
                    "edge_feat": np.array(
                        [self.ONEHOT_EDGETYPE[edge[2]] for edge in edges_reindexed]
                    ),
                    "node_feat": np.array(
                        [
                            [int(x) for x in list(graph_only_refs[key].values())[2:]]
    ...
    def generate_graphs(self) -> None:
        ...
        genes_to_construct = [
            ...
            if not (
                os.path.exists(f"{self.graph_dir}/{gene}_{self.tissue}")
...
def main() -> None:
    ...
    graphconstructingObject = GraphConstructor(
        params=params, genes=tpm_filtered_genes, graph_type="concatenated"
    )
