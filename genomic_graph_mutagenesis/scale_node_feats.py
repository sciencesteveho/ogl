...
def main() -> None:
    ...
    if args.graph_type == 'base':
        with open(f'{graph_dir}/{args.tissue}_base_graph.pkl', 'rb') as f:
            g = pickle.load(f)
        node_feat = g['node_feat']
        for i in range(0, 36):
            node_feat[:,i] = scalers[i].transform(node_feat[:,i].reshape(-1,1)).reshape(1, -1)[0]
        g['node_feat'] = node_feat 
        with open(f'{out_dir}/all_tissue_base_graph.pkl', 'wb') as output:
            pickle.dump(g, output)
    else:
        with open(f'{root_dir}/graphs/all_tissue_full_graph.pkl', 'rb') as f:
            g = pickle.load(f)
        node_feat = g['node_feat']
        for i in range(0, 36):
            node_feat[:,i] = scalers[i].transform(node_feat[:,i].reshape(-1,1)).reshape(1, -1)[0]
