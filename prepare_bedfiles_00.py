...
class GenomeDataPreprocessor:
    ...
    @time_decorator
    def _prepare_ensembl(self, bed_1: str, bed_2: str) -> None:
        """Remove INACTIVE, open chromatin regions, and CTCF from ensemble and combine with enhancer atlas"""
        cmd = f"gff2bed < {self.root_tissue}/unprocessed/{bed_1} \
            | grep -v -e 'activity=INACTIVE\|open_chromatin_region\|CTCF_binding_site' \
            | sed -e 's/^/chr/g' -e 's/;/\t/g' -e 's/activity=//g' \
            | awk -v FS='\t' -v OFS='\t' '{{print $1, $2, $3, $8\"_\"$10}}' \
            > {self.root_tissue}/unprocessed/{bed_1}.bed"

        cat_cmd = f"cat {self.root_tissue}/unprocessed/{bed_1}.bed {self.root_tissue}/interaction/{bed_2}.tabbed \
            | sort -k1,1 -k2,2n \
            > {self.root_tissue}/local/regulatorybuild_plus_enhancers_{self.tissue}.bed"

        for command in [cmd, cat_cmd]:
